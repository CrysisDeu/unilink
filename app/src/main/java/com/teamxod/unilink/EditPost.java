...
*/
public class EditPost extends AppCompatActivity implements IPickResult, DatePickerDialog.OnDateSetListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addpic.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // pickImage
                dialog = PickImageDialog.build(new PickSetup()).show(EditPost.this);
            }
        });
        ...
        if (old_post == null)
            System.out.println("ERR: Cannot get the house from firebase");
        else { // set UI elements according to firebase

            title.setText(old_post.getTitle());
            // TODO
          /*  street.setText(old_post.get);
            city = findViewById(R.id.city);
...
*/
    ...
    private boolean validation() {
        if (!filledIn || start_date.getText().toString().length() == 0) {
            Toast.makeText(EditPost.this, INVALID_FORM_DUE_TO_FILLING, Toast.LENGTH_LONG).show();
            return true;
        }
        if (roomBoxList.isEmpty() || photoBoxList.isEmpty()) {
            Toast.makeText(EditPost.this, INVALID_FORM_DUE_TO_ADDING, Toast.LENGTH_LONG).show();
            return true;
        }
        if (_houseType.length() == 0 || _bedroom_number.length() == 0
                || _bathroom_number.length() == 0 || _leaseLength.length() == 0) {
            Toast.makeText(EditPost.this, INVALID_FORM_DUE_TO_SELECTING, Toast.LENGTH_LONG).show();
            return true;
        }
        for (Room r : roomList) {
